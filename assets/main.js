...
(function () {
  ...
  /* --- form contatti -> WhatsApp (ottimizzato per conversioni) --- */
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      // Messaggio ottimizzato per conversioni: pulito, strutturato, professionale
      var lines = ["Ciao! 👋 Vorrei un preventivo gratuito per un lavoro.\n"];
      if (servizio && servizio !== "Altro") lines.push("🛠 Servizio richiesto: " + servizio);
      if (zona) lines.push("📍 Zona: " + zona);
      if (messaggio) lines.push("📝 Dettagli: " + messaggio);
      if (nome) lines.push("👤 Nome: " + nome);
      if (recapito) lines.push("📞 Recapito: " + recapito);
